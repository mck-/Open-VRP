...
(defgeneric run-algo (problem algo)
  (:method (problem algo)
    "Either problem or algo is not defined/correct")
  (:documentation "Runs the algo on the problem. Destructive - will have side-effects on the problem object. Use solve-prob to prevent this. Will return the <Algo> object, which will contain the solution (in the form of a copy of the <problem> object)."))
...
;; Currently it it defined in each run-algo defmethod. Should be general?
...
(defgeneric solve-prob (problem algo)
  (:method (problem algo)
    "Either problem or algo is not defined/correct")
  (:documentation "Solves the problem with the algo. Uses run-algo, but leaves the <problem> object untouched. Works with a clone (clone-problem in clone.lisp). NON-destructive wrapper to the run-algo method."))
...
(defgeneric solve-plot (problem algo)
  (:method (problem algo) "Requires two input objects: <problem>, <algo>")
...
(defgeneric re-init (obj)
  (:method (obj) "Cannot reset object of this type!")
